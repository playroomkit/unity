...
namespace UBB
{
    ...
    public class UnityBrowserBridge : MonoBehaviour
    {
        ...
        private readonly Queue<KeyValuePair<string, string>> messagesWithoutValue =
            new Queue<KeyValuePair<string, string>>();
        private readonly Queue<KeyValuePair<string, KeyValuePair<string, object>>> messagesWithValue =
            new Queue<KeyValuePair<string, KeyValuePair<string, object>>>();
        ...
        public static void CreateInstance()
        {
            if (_instance == null)
            {
                ...
                {
                    Debug.LogError(
                        "No UnityBrowserBridge component found in the scene. Please ensure it is attached to a GameObject.");
                    return;
                }
            }
        }
        ...
        public void StartUBB()
        {
            ...
            switch (browser)
            {
                case Browser.Chrome:
                    ...
                    webDriver = string.IsNullOrEmpty(webDriverDirectory)
                        ? new ChromeDriver(chromeOptions)
                        : new ChromeDriver(webDriverDirectory, chromeOptions);
                    break;
                case Browser.Edge:
                    ...
                    webDriver = string.IsNullOrEmpty(webDriverDirectory)
                        ...
                        : new EdgeDriver(webDriverDirectory, edgeOptions);
                    break;
                case Browser.Firefox:
                    ...
                    webDriver = string.IsNullOrEmpty(webDriverDirectory)
                        ...
                        : new FirefoxDriver(webDriverDirectory, firefoxOptions);
                    break;
                case Browser.Opera:
                    ...
                    webDriver = string.IsNullOrEmpty(webDriverDirectory)
                        ...
                        : new OperaDriver(webDriverDirectory, operaOptions);
                    break;
                case Browser.Safari:
                    SafariOptions safariOptions = new SafariOptions();
                    if (browserArguments.Length > 0)
                        Debug.LogWarning(
                            "Unity Browser Bridge - Safari Web Driver does not allow adding generic arguments. Please configure SafariOptions directly.");
                    webDriver = string.IsNullOrEmpty(webDriverDirectory)
                        ? new SafariDriver(safariOptions)
                        : new SafariDriver(webDriverDirectory, safariOptions);
                    break;
            }
        ...
        public void ExecuteJS(string jsCommand)
        {
            ...
            try
            {
                ((IJavaScriptExecutor)webDriver).ExecuteScript("_ubb_logBrowserCall('" +
                                                               jsCommand.Replace("'", "&apos;") + "')");
            ...
            {
                Debug.LogError(
                    "Unity Browser Bridge - Could not execute command <i>" + jsCommand + "</i>: " + e.Message);
            }
        }
        ...
        public T ExecuteJS<T>(string jsCommand)
        {
            ...
            {
                Debug.LogError("Unity Browser Bridge - Could not convert return value from <i>" + jsCommand +
                               "</i> to type " + typeof(T));
                return default;
            }
        }
        ...
        {
            lock (messagesWithValue)
                messagesWithValue.Enqueue(new KeyValuePair<string, KeyValuePair<string, object>>(gameObjectName,
                    new KeyValuePair<string, object>(methodName, value)));
        }
