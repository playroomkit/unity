...
public class PlayerEntitlement<TMetadata>
{
    ...
    private static PlayerEntitlement<TMetadata> ParsePlayerEntitlement(JSONNode node, Func<string, TMetadata> metadataParser)
    {
    ...
    public static List<PlayerEntitlement<TMetadata>> ParsePlayerEntitlements(string jsonString, Func<string, TMetadata> metadataParser)
    {
        List<PlayerEntitlement<TMetadata>> entitlements = new();
        JSONNode root = JSON.Parse(jsonString);

        if (!root.IsArray)
            Debug.LogWarning("Expected an array of of Entitlements");

        foreach (JSONNode item in root.AsArray)
        {
            var data = ParsePlayerEntitlement(item, metadataParser);
